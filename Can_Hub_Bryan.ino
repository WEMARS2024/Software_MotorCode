...
void loop() {
  ...
  if (xQueueReceive(CAN_cfg.rx_queue, &rx_frame, 3 * portTICK_PERIOD_MS) == pdTRUE) {
   ...
   if (rx_frame.MsgID == 0x050C1801) {
        uint8_t *p = (uint8_t *)&latitude;
        p[0] = rx_frame.data.u8[0];
        p[1] = rx_frame.data.u8[1];
        p[2] = rx_frame.data.u8[2];
        p[3] = rx_frame.data.u8[3];
        p[4] = rx_frame.data.u8[4];
        p[5] = rx_frame.data.u8[5];
        p[6] = rx_frame.data.u8[6];
        p[7] = rx_frame.data.u8[7];
        receivedLat = true;
      }

   if (rx_frame.MsgID == 0x050C1841) {
